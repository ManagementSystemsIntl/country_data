...
ggplot(byDay, aes(day, cases, color=fct_rev(country2))) + 
  ...
  scale_color_viridis_d() + 
  scale_x_datetime(limits=c(as.POSIXct("2020-01-22"), as.POSIXct("2020-07-07")),
...
ggsave("viz/top country cases 7 July 2020 legend.png",
       device="png",
       type="cairo",
       height=6,
       width=6)
...
bd <- cases %>%
  ...
  group_by(week) %>%
  mutate(week_new = round(mean(new_cases, na.rm=T),1)) %>% # rolling average by week, so both forward and backward looking window
  mutate(week_new_roll = round(roll_mean(new_cases, 7, align='right', fill=NA),1)) %>% # rolling average of preceding six days
  filter(day>as.POSIXct("2020-02-29")) %>%
  arrange(day)
...
bd_wk <- bd %>%
  group_by(week) %>%
  summarize(cases=mean(cases, na.rm=T),
            new_cases=mean(new_cases, na.rm=T))
  
head(bd_wk)


ggplot(bd, aes(day, cases)) + 
  stat_smooth(size=1, alpha=.2, se=F, span=.1) + 
  scale_color_viridis_d() + 
  scale_x_datetime(limits=c(as.POSIXct("2020-03-01"), as.POSIXct("2020-07-07")),
                   breaks=date_breaks("1 month"),
                   labels=date_format("%b-%d")) +
  ...
  labs(x="",
       y="",
       title="COVID-19 cases, Bangladesh\n1 March - 7 July, 2020") + 
  spec + 
  theme(legend.title=element_blank(),
        panel.border=element_blank(),
        axis.ticks=element_blank())
...
bng <- read_excel("csse_covid_19_data/csse_covid_19_time_series/Bangladesh covid (8 July 2020).xlsx")
...
ggplot(bng, aes(day)) + 
  stat_smooth(aes(y=tested), color="darkblue", size=1, alpha=.2, se=F, span=.1) + 
  stat_smooth(aes(y=cases), color="darkgoldenrod3", size=1, alpha=.2, se=F, span=.1) + 
  stat_smooth(aes(y=deaths), color="maroon", size=1, alpha=.2, se=F, span=.1) + 
  stat_smooth(aes(y=recovered), color="purple", size=1, alpha=.2, se=F, span=.1) + 
  stat_smooth(aes(y=new_cases), color="cyan", size=1, alpha=.2, se=F, span=.1) + 
  scale_x_datetime(limits=c(as.POSIXct("2020-03-01"), as.POSIXct("2020-07-07")),
                   breaks=date_breaks("1 month"),
                   labels=date_format("%b-%d")) +
  scale_y_log10(labels=comma)
...
ggplot(bngL, aes(day, value, color=type)) + 
  stat_smooth(se=F) + 
  scale_color_viridis_d() +
  scale_x_datetime(limits=c(as.POSIXct("2020-03-01"), as.POSIXct("2020-07-08")),
                                             breaks=date_breaks("1 month"),
                                             labels=date_format("%b")) +
  #scale_y_log10(labels=comma) + 
  scale_y_continuous(trans=log2_trans(),
                     breaks=c(1,50,1000,32000,1000000),
                     labels=comma) +
  nobord +
  labs(x="",
       y="",
...
ggsave("viz/COVID in Bangladesh (8 Jul 2020).png",
       device="png",
       type="cairo",
       height=4,
       width=7)
